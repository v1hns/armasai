...
const STAGES = [
  ...
  { id: "requirements", label: "② Requirements",        desc: "Derive engineering spec" },
  { id: "design",       label: "③ CAD Design",          desc: "Generate arm candidates" },
  { id: "cad",          label: "④ CAD Generation",      desc: "Build STL + MJCF" },
  { id: "sim_eval",     label: "⑤ Physics Simulation",  desc: "MuJoCo stress testing" },
...
function markStageDone(id, elapsed) {
  ...
  if (sel && elapsed) sel.textContent = elapsed < 1 ? `${(elapsed * 1000).toFixed(0)}ms` : `${elapsed.toFixed(1)}s`;
...
function renderStageContent(id, data) {
  switch (id) {
    case "perception":    return renderPerception(data);
    case "requirements":  return renderRequirements(data);
    case "design":        return renderDesign(data);
    case "cad":           return renderCad(data);
    case "sim_eval":      return renderSimEval(data);
    case "rl_loop":       return renderRLLoop(data);
    case "final":         return renderFinal(data);
  }
}
...
function renderRequirements(d) {
  ...
  let romRows = Object.entries(rom).map(([j, [lo, hi]]) =>
    `<div class="bar-row">
...
// RL chart state
const _rlSteps = [], _rlRewards = [];
let _rlCanvas = null;

function renderRLLoop(d) {
  renderStageBody("rl_loop", `
...
function updateRLChart(d) {
  ...
  document.getElementById("rl-ts") && (document.getElementById("rl-ts").textContent =
    d.timestep >= 1000 ? Math.round(d.timestep/1000)+"k" : String(d.timestep));
  document.getElementById("rl-rew") && (document.getElementById("rl-rew").textContent =
    (d.mean_reward || 0).toFixed(3));
...
function drawRLChart(canvas) {
  ...
  const xmin = _rlSteps[0], xmax = _rlSteps[_rlSteps.length-1] || 1;
  let ymin = Math.min(..._rlRewards), ymax = Math.max(..._rlRewards);
  if (ymin === ymax) { ymin -= 0.1; ymax += 0.1; }
  const xs = s => pad[3] + (s - xmin) / (xmax - xmin) * (W - pad[3] - pad[1]);
  const ys = v => pad[0] + (1 - (v - ymin) / (ymax - ymin)) * (H - pad[0] - pad[2]);
  ctx.strokeStyle = "#2a3742"; ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) {
    const y = pad[0] + i/4 * (H - pad[0] - pad[2]);
    ctx.beginPath(); ctx.moveTo(pad[3], y); ctx.lineTo(W - pad[1], y); ctx.stroke();
    const v = ymax - i/4 * (ymax - ymin);
    ctx.fillStyle = "#8aa0b0"; ctx.font = "9px Arial";
    ctx.fillText(v.toFixed(2), 2, y + 3);
  }
  ctx.strokeStyle = "#6fe6a0"; ctx.lineWidth = 2;
  ctx.beginPath();
  ...
  for (let i = 0; i < _rlSteps.length; i += xstep) {
    const ts = _rlSteps[i];
    ctx.fillText(ts >= 1000 ? Math.round(ts/1000)+"k" : String(ts), xs(ts)-8, H - 4);
  }
}
...
function updateStatsBar(stats) {
  ...
  set("s-mass", stats.total_mass_g !== undefined ? stats.total_mass_g.toFixed(0)+"g" : "—");
  set("s-reach", stats.reach_envelope_mm !== undefined ? stats.reach_envelope_mm+"mm" : "—");
  set("s-dof", stats.dof !== undefined ? String(stats.dof) : "—");
  set("s-ik", stats.ik_success_rate !== undefined ? (stats.ik_success_rate*100).toFixed(0)+"%" : "—");
  set("s-rl", stats.rl_success_rate !== undefined ? (stats.rl_success_rate*100).toFixed(0)+"%" : "—");
  const life = stats.predicted_life_years;
  set("s-life", life !== undefined && life < 900 ? life.toFixed(1)+"yr" : (life !== undefined ? "≥100yr" : "—"));
  set("s-energy", stats.mean_energy_j !== undefined ? stats.mean_energy_j.toFixed(0)+"J" : "—");
...
async function loadClips() {
  try {
    ...
    grid.innerHTML = "";
    if (!clips.length) {
      grid.innerHTML = '<div style="color:var(--dim);font-size:12px;padding:8px">No clips found in test_vids/</div>';
      return;
    }
...
function markStageError(id, msg) {
  ...
  renderStageBody(id, `<div style="color:var(--danger);font-size:12px;padding:4px 0">${msg}</div>`);
}
...
async function initViewer() {
  try {
    ...
    key.castShadow = true;
    scene.add(key);
    scene.add(Object.assign(new THREE.DirectionalLight(0x99bbff, 0.6), { position: new THREE.Vector3(-2, 2, -1) }));
    ...
    document.getElementById("viewer-loading").textContent = "Viewer unavailable: " + err;
  }
}
...
async function loadViewerScene(name) {
  try {
    const xmlText = await fetch(`/api/scene?name=${encodeURIComponent(name)}`).then(r => {
      if (!r.ok) throw new Error(r.status);
      return r.text();
    });

    // Clean up old model
    if (mjData) { mjData.free?.(); mjData = null; }
    if (mjModel) { mjModel.free?.(); mjModel = null; }
  ...
  for (let g = 0; g < model.ngeom; g++) {
    ...
    if (type === (G.mjGEOM_SPHERE?.value ?? 2)) {
      ...
      if (!(meshID in meshes)) {
        ...
        for (let v = 0; v < vert.length; v += 3) { const t = vert[v+1]; vert[v+1] = vert[v+2]; vert[v+2] = -t; }
    ...
    mesh.quaternion.set(-gq[g*4+1], -gq[g*4+3], gq[g*4+2], gq[g*4]);
  }
  return bods;
}
...
function updateBodies() {
  for (let b = 0; b < mjModel.nbody; b++) {
    ...
    const xp = mjData.xpos, xq = mjData.xquat;
    bodies[b].position.set(xp[b*3], xp[b*3+2], -xp[b*3+1]);
    bodies[b].quaternion.set(-xq[b*4+1], -xq[b*4+3], xq[b*4+2], xq[b*4]);
